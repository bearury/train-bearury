...
export class CarriageFirestoreService {
  ...
  public addCarriage(
    carriageValue: CarriageFormValue
  ): Observable<string | null> {
    this.loaderInPageService.show();
    const stationRef = collection(this.db, 'carriages');

    return from(
      addDoc(stationRef, this.transformTypeCarriage(carriageValue))
    ).pipe(
      switchMap(docRef => {
      ...
      tap(id => {
        this.alert.open('Новый вагон успешно создан!').subscribe();
        this.loaderInPageService.hide();
        return of(id);
      }),
      catchError(err => {
        this.alert.open('Ошибка создания вагона!', err).subscribe();
        this.loaderInPageService.hide();
        return of(null);
      })
    );
  ...
  public updateCarriage(
    ...
    return from(
      updateDoc(carriageRef, this.transformTypeCarriage(carriageValue))
    ).pipe(
      map(() => {
  ...
  private transformTypeCarriage(
    carriageValue: CarriageFormValue
  ): Omit<CarriageEntity, 'id'> {
    const { name, rows, leftSeats, rightSeats, backLeftSeats, backRightSeats } =
      carriageValue;
    return {
      rows: rows ?? 0,
      name: name ?? '',
